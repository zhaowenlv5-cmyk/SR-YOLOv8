...
def feature_visualization(x, module_type, stage, n=32, save_dir=Path("runs/detect/exp")):
    ...
    if height > 1 and width > 1:
        ...
        single_filename_dir = os.path.join(save_dir, f"stage{stage}_{module_type.split('.')[-1]}_features") #lzw
        if not os.path.exists(single_filename_dir): #lzw
            os.makedirs(single_filename_dir) #lzw
        for i in range(n):
            ax[i].imshow(blocks[i].squeeze())  # cmap='gray'
            ax[i].axis("off")
            single_filename = os.path.join(single_filename_dir, f"channel{i}.png") #lzw
            plt.imsave(single_filename, blocks[i].squeeze().numpy()) #lzw
